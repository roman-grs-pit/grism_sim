...
def setup_pysiaf(wfi_cen_ra: float, wfi_cen_dec: float, wfi_cen_pa: float, det_num: int):
    ...
    # pysiaf wfi_cen_pa is 60 more than image_utils wfi_cen_pa (i.e. siaf_pa = iu_pa + 60)
    attmat = pysiaf.utils.rotations.attitude_matrix(
        v2ref, v3ref, wfi_cen_ra, wfi_cen_dec, wfi_cen_pa)

    wfi_siaf.set_attitude_matrix(attmat)

    # cache as global variables
    _WFI_SIAF = wfi_siaf
    # I believe pysiaf uses 0-index for origin pixel; thus, center pix is 2043 not 2044
    _DET_RA, _DET_DEC = wfi_siaf.det_to_sky(2043, 2043)
...
def trim_catalog(catalog, wfi_cen_ra, wfi_cen_dec, wfi_cen_pa, det_num, gpad, tot_im_size,
    ...
    object_xy_siaf = wfi_siaf.sky_to_sci(
        trimmed_catalog[ra_col], trimmed_catalog[dec_col])
    ...
    # stars00['Xpos'] < 4088 + 2*( gpad) #we want everything within padded area around grism
    sel_ondet = object_xy[0] > 0
    sel_ondet &= object_xy[0] < tot_im_size
    # stars00['Xpos'] < 4088 + 2*( gpad) #we want everything within padded area around grism
    sel_ondet &= object_xy[1] > 0
...
def star_postage_grid(psf_grid, mag, detx=2044, dety=2044, half_fov_pixels=182):
    ...
    x_0 = int(detx)
    y_0 = int(dety)
    y, x = np.mgrid[y_0-half_fov_pixels:y_0+half_fov_pixels,
                    x_0-half_fov_pixels:x_0+half_fov_pixels]

    psf = psf_grid.evaluate(x=x, y=y, x_0=detx, y_0=dety,
...
def gal_postage_grid(psf_grid, detx=2044, dety=2044, half_fov_pixels=364, flux=1):
    ...
    # ? What's the right flux for this?
...
def create_psf_grid(wavelength=1.5e-6, fov_pixels=364, det="SCA01"):
    ...
    wfi.detector = det
    grid = wfi.psf_grid(all_detectors=False, use_detsampled_psf=True,
                        monochromatic=wavelength, fov_pixels=fov_pixels)
    return grid
...
def star_postage(mag, detx=2044, dety=2044, offx=0, offy=0, wavelength=1.5e-6, fov_pixels=364, oversample=4, arcperpixel=0.11, det="SCA01"):
    ...
    psf = wfi.calc_psf(monochromatic=wavelength,
...
def add_Roman_header(file_name, fout_name=None, roman_base_dir='', det='1'):
    ...
    file[1].header["CONFFILE"] = roman_base_dir + \
        "configuration/Roman.det"+det+".07242020.conf"
