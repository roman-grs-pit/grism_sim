# Same as grism_sim_psf_dependent.py but using new Galacticus mocks
...
# Addition for new Galacticus mocks
import sys
sys.path.append(os.environ['github_dir']+'/galacticus_sed_calculator')
from astropy.cosmology import FlatLambdaCDM
import astropy.units as u
# Setup new SED
cosmology_unit = FlatLambdaCDM(H0=67.74, Om0=0.3089)
...
sedCalc = sed.sed_calculator(sedTemplateFilename, cosmology=cosmology_unit)
#from galacticus_sed_calculator import SEDCalculator  # Gives import error: ModuleNotFoundError: No module named 'galacticus_sed_calculator'
#sedCalc = SEDCalculator(sedTemplateFilename, cosmology=cosmology_unit) 
...
def mk_grism(wfi_cen_ra,wfi_cen_dec,wfi_cen_pa,det_num,star_input,gal_input,output_dir,
             ...
             gal_mag_col='mag_F158_app',dogal='y',magmax=25,
    ...
    catalog_type = []  # Type of object (0 for star and 1 for gal)
    catalog_mag = []   # Magnitude of object
    catalog_SED_info = []   # SED information for object
    # Galaxy-specific catalog info
    catalog_z = []     # Redshift of object
    catalog_F158_mag_AB = [] # Absolute Magnitude in F158 filter
    catalog_flux_halpha = [] # True line flux for H-alpha
    catalog_flux_oiii4933 = []   # True line flux for OIII[4933]
    catalog_flux_oiii4960 = []   # True line flux for OIII[4960]
    catalog_flux_oiii5008 = []   # True line flux for OIII[5008]
    catalog_spheroid_radius = [] # Size of source buldge
    catalog_disk_radius = []     # Size of source disk
    ...
    catalog_type.extend([0]*nstar)
    catalog_SED_info.extend(stars['star_template_index'])
    # Galaxy specific columns are just set to 0 for stars
    catalog_z.extend([0]*nstar)
    catalog_F158_mag_AB.extend([0]*nstar)
    catalog_flux_halpha.extend([0]*nstar)
    catalog_flux_oiii4933.extend([0]*nstar)
    catalog_flux_oiii4960.extend([0]*nstar)
    catalog_flux_oiii5008.extend([0]*nstar)
    catalog_spheroid_radius.extend([0]*nstar)
    catalog_disk_radius.extend([0]*nstar)
    ...
    if dogal == 'y':
        ...
        catalog_type.extend([1]*ngal)
        catalog_SED_info.extend(gals['unique_ID'])
        catalog_z.extend(gals['Z'])
        catalog_F158_mag_AB.extend(gals['mag_F158_abs'])
        catalog_flux_halpha.extend(gals['flux_halpha'])
        catalog_flux_oiii4933.extend(gals['flux_oiii4933'])
        catalog_flux_oiii4960.extend(gals['flux_oiii4960'])
        catalog_flux_oiii5008.extend(gals['flux_oiii5008'])
        catalog_disk_radius.extend([r_eff]*ngal)
        catalog_spheroid_radius.extend([r_eff]*ngal)
    ...
    # * Save SED input fluxes, true line fluxes, and source sizes
    ...
    if ngal>0:
        for ii in range(0, ngal):
            row = gals[ii]            
            sim_fn = os.path.join('/global/cfs/cdirs/m4943/Galacticus/mockCatalogs/paper1Calibration_allUNIT_0.2degMock/', 'romanUNIT-d' + str(row['SIM']) + '_0.2Deg.hdf5')
            obs_wavelengths = np.linspace(0.5, 2.5, 1000) * u.micron # Wavelengths to calculate SED over
            high_res_wavelengths = np.arange(7e3, 2e4, 5) * u.angstrom
            total_flux = sedCalc.evaluate_total_spectrum(sim_fn, galIndex=row['IDX'], obs_wavelengths=obs_wavelengths, use_synphot=False)
            sed_flux = total_flux(high_res_wavelengths, flux_unit='FLAM') # This gets flux interpolating for high res
            catalog_SED_flux.append(sed_flux)
    ...
    detector_level_catalog = Table([catalog_index, catalog_ra, catalog_dec, catalog_detx, catalog_dety, 
                                    catalog_mag, catalog_type, catalog_SED_info, catalog_z, catalog_F158_mag_AB, 
                                    catalog_flux_halpha, catalog_flux_oiii4933, catalog_flux_oiii4960, catalog_flux_oiii5008, 
                                    catalog_spheroid_radius, catalog_disk_radius], 
                                    names=['INDEX', 'RA', 'DEC', 'DET_X', 'DET_Y',
                                           'MAG', 'CLASSIFICATION', 'SED_INFO', 'TRUE_REDSHIFT', 'F158_mag_AB',
                                           'TRUE_FLUX_HALPHA', 'TRUE_FLUX_OIII4933', 'TRUE_FLUX_OIII4960', 'TRUE_FLUX_OIII5008', 
                                           'SIZE_SPHEROID', 'SIZE_DISK'],
    ...
    # Write detector level catalog as image HDU
    hdu_catalog = fits.table_to_hdu(detector_level_catalog)
    hdu_catalog.name = "CATALOG"
    hdu_catalog.header["PADDING"] = gpad

    # Write detector level catalog as parquet file
    detector_level_catalog_filename = os.path.join(output_dir, 'catalog'+extra_grism_name+'_'+det+'.parquet')
    detector_level_catalog.meta['comments'] = ['Padding:', gpad]
    detector_level_catalog.write(detector_level_catalog_filename, format='parquet', overwrite=True)

    # Store wavelength grid for galaxies
    wave_star = np.arange(7000, 20005, 5)  # Angstroms
    wave_gal = np.linspace(7000, 20005, 5) # Angstroms
    ...
    SED_df.meta['comments'] = ['Wavelength (stars)', str(wave_star), 
    ...
    for ii, start_wave in enumerate(bins[:-1]):
        ...
        if ngal > 0:
            ...
            for jj in gal_iter:
                ...
                sim_fn = os.path.join('/global/cfs/cdirs/m4943/Galacticus/mockCatalogs/paper1Calibration_allUNIT_0.2degMock/', 'romanUNIT-d' + str(row['SIM']) + '_0.2Deg.hdf5')
                obs_wavelengths = np.linspace(0.5, 2.5, 1000) * u.micron # Wavelengths to calculate SED over
                high_res_wavelengths = np.arange(1e4, 2e4, 5) * u.angstrom
                total_flux = sedCalc.evaluate_total_spectrum(sim_fn, galIndex=row['IDX'], obs_wavelengths=obs_wavelengths, use_synphot=False)
                sed_flux = total_flux(high_res_wavelengths, flux_unit='FLAM') # This gets flux interpolating for high res
                
                # initial cut to avoid errors from nan values
                sel_wave = high_res_wavelengths.value > minlam
                sel_wave &= high_res_wavelengths.value < maxlam
                wave = high_res_wavelengths.value[sel_wave]
                flux = sed_flux.value[sel_wave]
                ...
                start = time.time()
                segment_of_dispersion = roman.compute_model_orders(id=photid, mag=mag, compute_size=False, size=size, in_place=False, store=False, is_cgs=True, spectrum_1d=[wave, flux])

                # Should this be used instead?
                #segment_of_dispersion = roman.compute_model_orders(id=photid, mag=mag, compute_size=False, size=size, in_place=False, store=False, is_cgs=True, spectrum_1d=[high_res_wavelengths.value, sed_flux.value])
