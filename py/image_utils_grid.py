...
grid = wfi.psf_grid(all_detectors=False, use_detsampled_psf=True, monochromatic=1.5e-6, fov_pixels=364)

def star_postage_grid(mag, detx=2044, dety=2044, wavelength=1.5e-6, fov_pixels=364):
    flux = mag2flux(mag)
    half_fov = fov_pixels
    y, x = np.mgrid[dety-half_fov:dety+half_fov, detx-half_fov:detx+half_fov]
    psf = grid.evaluate(x=x, y=y, x_0=detx, y_0=dety, flux=flux)
