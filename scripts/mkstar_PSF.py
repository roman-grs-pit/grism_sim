...
import wavelength_dependent_PSF_grism_sim as psfiu
...
for ii, start_wave in enumerate(bins[:-1]):
    ...
    if args.mkdirect == 'y':
        ...
        for i in range(0,len(stars00)):
            ...
            if args.fast_direct == 'y':
                ...
                sp = iu.star_postage(mag,xpos,ypos,xoff,yoff,fov_pixels=pad-1,det=det, wavelength=(start_wave * (10**-10)))
            fov_pixels = pad-1
    ...
    for i in range(0,len(stars00)):
        ...
        spec = psfiu.interpolate_spectrum(spec)
        wave, flux = psfiu.split_and_apodize(spec, start_wave, end_wave, grizli_conf["spectrum_overlap"])
    ...
    grism_collector.append(roman.model)
#! End loop

total_grism = np.zeros_like(roman.model)
for ii in range(len(grism_collector)):
    total_grism += grism_collector[ii]
roman.model = total_grism
